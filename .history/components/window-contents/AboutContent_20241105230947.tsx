...
function SkillC<SkillCard title="Languages" skills="Python, JavaScript, TypeScript, C++, PHP, HTML, CSS, SQL" />
...
<SkillCard title="Tools" skills="Git, JIRA, Confluence, Postman, MySQL Workbench, MongoDB Atlas, VSCode, Jupyter Notebook, Jira, Selenium" />ard({ title, icons }: SkillCardProps) {
  return (
    <Card>
      <CardHeader>
        ...
        <div className="flex flex-wrap gap-3">
          {icons.map((skill, index) => (
            <div key={index} className="relative group">
              <FontAwesomeIcon icon={skill.icon} className="w-6 h-6" />
              <span className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 text-xs bg-background border text-foreground rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                {skill.label}
              </span>
            </div>
          ))}
...
function SkillsSection() {
  ...
  const frontendIcons: SkillIcon[] = [
    { icon: faReact, label: 'React' },
    { icon: faReact, label: 'Next.js' },
    { icon: faCode, label: 'TypeScript' },
    { icon: faCode, label: 'Electron' },
    { icon: faCss3Alt, label: 'Tailwind CSS' },
    { icon: faCode, label: 'Three.js' },
  ]

  const backendIcons: SkillIcon[] = [
  ...
  const toolIcons: SkillIcon[] = [
    { icon: faGit, label: 'Git' },
    { icon: faJira, label: 'JIRA' },
    { icon: faConfluence, label: 'Confluence' },
    { icon: faNetworkWired, label: 'Postman' },
    { icon: faDatabase, label: 'MySQL Workbench' },
    { icon: faDatabase, label: 'MongoDB Atlas' },
    { icon: faCode, label: 'VSCode' },
    { icon: faPython, label: 'Jupyter Notebook' },
    { icon: faCode, label: 'Selenium' },
  ]
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <SkillCard title="Frontend" icons={frontendIcons} />
        <SkillCard title="Backend" icons={backendIcons} />
        <SkillCard title="Tools" icons={toolIcons} />
      </div>
    </section>
  )
}