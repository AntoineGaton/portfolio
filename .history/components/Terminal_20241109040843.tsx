...
`.trim();
      ...
      case 'js':
      case 'javascript':
        if (args.length < 2) return 'Please provide JavaScript code to execute';
        return executeCode('javascript', args.slice(1).join(' '));

      case 'py':
      case 'python':
        if (args.length < 2) return 'Please provide Python code to execute';
        return executeCode('python', args.slice(1).join(' '));
