...
export function Window({
  ...
  const [isFullscreen, setIsFullscreen] = useState(defaultIsFullscreen || false);
  ...
  // Debug visibility
  useEffect(() => {
    console.log(`Window ${id} rendered:`, {
      isActive,
      isMinimized,
      isFullscreen,
      position: initialPosition
    });
  }, [id, isActive, isMinimized, isFullscreen, initialPosition]);
  ...
  return (
    <Card
      ...
      className={cn(
        ...
        isFullscreen ? "w-screen h-[calc(100vh-48px)]" : "",
        isActive ? "z-50" : "z-40",
        "border border-primary" // Add visible border for debugging
      )}
      style={{
        transform: isFullscreen ? 'none' : `translate(${initialPosition?.x || 0}px, ${initialPosition?.y || 0}px)`,
        width: isFullscreen ? '100vw' : (initialPosition?.width || '800px'),
        height: isFullscreen ? 'calc(100vh - 48px)' : (initialPosition?.height || '600px'),
