...
export const MobileLayout = () => {
  ...
  useEffect(() => {
    if (iconPositions.length === 0) {
      const initialPositions = apps.map((app, index) => ({
        id: app.id,
        x: (index % 4) * 100 + 16,
        y: Math.floor(index / 4) * 100 + 16
      }));
      setIconPositions(initialPositions);
    }
  }, [apps, iconPositions.length]);
  ...
  const handleDrag = (e: React.DragEvent, id: string) => {
    ...
    // Account for padding (16px = p-4) and full icon size (80px)
    const maxX = bounds.width - 80 - 16; // container width - icon width - right padding
    const maxY = bounds.height - 80 - 16; // container height - icon height - bottom padding
    
    const x = Math.max(16, Math.min(e.clientX - bounds.left - 40, maxX));
    const y = Math.max(16, Math.min(e.clientY - bounds.top - 40, maxY));
  ...
  return (
    <div className="min-h-screen overflow-hidden touch-none">
      ...
      <div className="relative min-h-screen overflow-hidden">
        ...
        {/* App grid */}
        <div className={cn(
          "relative h-[calc(100vh-4rem)] overflow-hidden touch-none p-4",
          openApp && "hidden"
        )}>
          {apps.map((app) => {
            const Icon = app.icon;
            const position = iconPositions.find(pos => pos.id === app.id);
        ...
        {openApp && (
          <div
            className={cn(
              "fixed inset-0 bg-background overflow-hidden p-4",
            ...
            <div className="h-full overflow-hidden">
              ...
              <div className="sticky top-0 z-10 flex items-center justify-between p-4 bg-background/80 backdrop-blur-sm border-b">
                ...
                <h1 className="text-lg font-semibold">
                  {apps.find(app => app.id === openApp)?.component.name}
                </h1>
